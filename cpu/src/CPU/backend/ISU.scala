...
class ISU(val parameter: CPUParameter) 
  ...
  io.out.bits.data.src(0) := Mux(SrcType.isReg(io.in.bits.ctrl.srcType(0)), Mux(canforward(0), io.forward.rfData, io.rfread.port(0).data), io.in.bits.data.src(0))
  io.out.bits.data.src(1) := Mux(SrcType.isReg(io.in.bits.ctrl.srcType(1)), Mux(canforward(1), io.forward.rfData, io.rfread.port(1).data), io.in.bits.data.src(1))
