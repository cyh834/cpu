...
class Mul extends Module{
  ...
  a :=Mux(io.flush.asBool,0.U,
      Mux((io.in.valid&io.in.bits.mul_signed(1)&io.in.bits.mulw).asBool,Cat(Fill(100,io.in.bits.multiplicand(31)),io.in.bits.multiplicand),
      Mux((io.in.valid&io.in.bits.mul_signed(1)&(~io.in.bits.mulw)).asBool,Cat(Fill(68,io.in.bits.multiplicand(63)),io.in.bits.multiplicand),
      Mux(io.in.valid.asBool,io.in.bits.multiplicand
      ,0.U))))
    
  b :=Mux(io.flush.asBool,0.U,
      Mux((io.in.valid&io.in.bits.mul_signed(1)&io.in.bits.mulw).asBool,Cat(Fill(100,io.in.bits.multiplier(31)),io.in.bits.multiplier),
      Mux((io.in.valid&io.in.bits.mul_signed(1)&(~io.in.bits.mulw)).asBool,Cat(Fill(68,io.in.bits.multiplier(63)),io.in.bits.multiplier),
      Mux(io.in.valid.asBool,io.in.bits.multiplier
      ,0.U))))

  valid :=Mux(io.flush.asBool,0.U,io.in.valid)

  val tree = Module(new walloc_64_mul).io
  tree.mul_valid:=valid
  tree.multiplicand:=a//io.in.bits.multiplicand
  tree.multiplier:=b//io.in.bits.multiplier
  io.out.bits.result_hi:=tree.result_hi
  io.out.bits.result_lo:=tree.result_lo
