...
class IFU(val parameter: CPUParameter)
  ...
  // 只有一半的指令
  val half_inst_valid = !isrvc && (io.in.bits.pc(2, 1) === 3.U)
  val half_inst_valid_reg = RegEnable(half_inst_valid, io.in.valid)
  val half_inst = RegEnable(inst(15, 0), io.in.valid)
  val half_pc = RegEnable(io.in.bits.pc, io.in.valid)

  io.out.bits.pc := Mux(half_inst_valid_reg, half_pc, io.in.bits.pc)
  ...
  io.out.valid := io.in.valid && !io.flush && !half_inst_valid
  io.flush_rvc := (isrvc || half_inst_valid || half_inst_valid_reg) && io.in.valid && !io.in.bits.pred_taken
  io.flush_pc := io.in.bits.pc + 2.U
}
