...
_CPU_SIM_RESULT_DIR=${CPU_SIM_RESULT_DIR:-"cpu-sim-result"}
_CURRENT="$_CPU_SIM_RESULT_DIR"/all/"$_NOW"
mkdir -p "$_CURRENT"
ln -sfn "all/$_NOW" "$_CPU_SIM_RESULT_DIR/result"
...
echo "VCS emulator finished, result saved in $_CPU_SIM_RESULT_DIR/result"
